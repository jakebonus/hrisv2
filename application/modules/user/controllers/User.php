...
class User extends MX_Controller {
			...
			if($this->input->post('username') && $this->input->post('password')){
				...
				if($result = $this->mdl_user->m_login($username,$password)){
					...
					foreach($result as $a)
					{
						...
						$this->session->set_userdata('is_depthead',$a->is_depthead);
						...
						$this->session->set_userdata('a_office',$a->a_office);
	...
	public function ajax_get_workinfo(){
		$a_id = $this->session->userdata('a_id');
		$result = $this->mdl_user->m_get_emp_work_exp($a_id);
		echo json_encode($result);
	}

	public function ajax_get_education(){
		$a_id = $this->session->userdata('a_id');
		$result = $this->mdl_user->m_get_emp_education($a_id);
		echo json_encode($result);
	}
	...
	public function add_employee_training(){
		...
		$training['t_type'] = $this->input->post('t_type');
	...
	public function add_employee_work(){
		...
		$ms1 = str_replace('₱','',$this->input->post('p_salarymonthly'));
		$work['p_salarymonthly'] = str_replace(',','',$ms1);
	...
	public function update_employee_work(){
		...
		$ms1 = str_replace('₱','',$this->input->post('m_p_salarymonthly'));
		$work['p_salarymonthly'] = str_replace(',','',$ms1);
		...
		$work['w_type'] = $this->input->post('m_w_type');
	...
	public function update_employee_training(){
		...
		$training['t_type'] = $this->input->post('m_t_type');
	...
	public function request_delete_employee_educ(){
		...
		$data['e_isdeleted'] = 'YES';
		$where_field = 'e_id';
		$where_val = $this->input->post('m_e_id');
		$table = 'tbl_educbg';

		if($this->mdl_user->m_delete($where_field,$where_val,$table,$data)){
			$result = array('status' => 'yes','content'=> 'Request for delete success submitted!');
				echo json_encode($result);
				exit;
		}else{
	...
	public function request_delete_employee_elig(){
		...
		$data['e_deleted'] = 'YES';
		$where_field = 'el_id';
		$where_val = $this->input->post('m_el_id');
		$table = 'tbl_eligibility';
	...
	public function user_delete_employee_work_exp(){
		...
		// if($this->mdl_user->m_request_delete($where,$table,$field,$forapproval_field,$forapproval)){
				...
				$data['is_deleted'] = 'YES';
				$where_field = 'w_id';
				$where_val = $this->input->post('m_w_id');
				$table = 'tbl_workinfo';
	...
	public function request_delete_employee_training(){
		...
		// if($this->mdl_user->m_request_delete($where,$table,$field,$forapproval_field,$forapproval)){
				...
				$data['t_deleted'] = 'YES';
				$where_field = 't_id';
				$where_val = $this->input->post('m_t_id');
				$table = 'tbl_training';
	...
	public function personnelrequest(){
		if($this->session->userdata('a_id')){
			$data['title'] = 'Employee';
			$data['content'] = 'user/v_personnelrequest';
			$this->load->view('layouts/v_master', $data);
		}else{
			redirect('user');
		}
	...
	public function save_personnelrequest(){

			$data['jm_office'] 			= $this->session->a_office;
			$data['jm_officehead']	 	= $this->session->userdata('a_id');

			$data['jm_reqdate'] 		= date('Y-m-d');
			$data['jm_project'] 		= $this->input->post('jm_project');

			$originalDate = $this->input->post('jm_dateneeded');
			$data['jm_dateneeded']  = date("Y-m-d", strtotime($originalDate));

			$data['jm_projectduration'] = $this->input->post('jm_projectduration');
			$data['jm_reqnature'] 		= $this->input->post('jm_reqnature');
			$data['jm_reqnature_specific'] = $this->input->post('jm_reqnature_specific');
			$data['jm_justification']	= $this->input->post('jm_justification');
			$data['jm_emp_status'] 		= $this->input->post('jm_emp_status');
			$data['jm_desc'] 			= $this->input->post('jm_desc');

			$data['jm_eligibility'] = $this->input->post('jm_eligibility');
			$data['jm_training'] = $this->input->post('jm_training');
			$data['jm_experience'] = $this->input->post('jm_experience');
			$data['jm_skills'] = $this->input->post('jm_skills');
			...
			$educations = $this->input->post('educ');
			// $data['jm_course'] = $this->input->post('jm_course');
			// $educations = $this->input->post('education');
			// $educations = $this->input->post('jm_course');
			// print_r($educations);
			// echo $educations;
			// die();
			$educ_count = count($educations);
			// echo $educations['0'];
			// echo $educ_count;
			// print_r($educations);
			// die();
			...
			for($i = 0; $i < $educ_count; $i++){
					...
					$data['jm_course'] .= $educations[$i].',';

			}
			// echo '<pre>';
			// print_r($data);
			// die();
			if($this->mdl_user->m_saverequest($data)){
				$result = array('status' => 'yes','content'=> 'Personnel request successfully save!');
					echo json_encode($result);
			}else{
				$result = array('status' => 'no','content'=> 'Personnel request failed to save!');
					echo json_encode($result);
			}
	...
	public function cancel_personnelrequest(){
		$id = $this->input->post('m_jm_id');
		$data['jm_iscancel'] = 'YES';
			if($this->mdl_user->m_updaterequest($id,$data)){
				$result = array('status' => 'yes','content'=> 'Personnel Request successfully cancelled!');
					echo json_encode($result);
			}else{
				$result = array('status' => 'no','content'=> 'Personnel Request failed to cancelled');
					echo json_encode($result);
			}
	}

	public function ajax_get_personnelrequestlist(){
		$result = $this->mdl_user->m_get_personnelrequestlist($this->session->userdata('a_id'));
			echo json_encode($result);
	...
	public function listofpersonnelrequest(){
		$data['title'] = 'Employee';
		$data['content'] = 'user/v_personnelrequestlist';
		$this->load->view('layouts/v_master', $data);
	}
