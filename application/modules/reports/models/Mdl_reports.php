...
class Mdl_reports extends CI_Model {
	...
	public function m_update_leave_printdate($l_id,$print_date){
		$sql = "UPDATE `tbl_leaves` SET `l_printeddate`= '".$print_date."', `l_processedby`='".$this->session->userdata('a_empno')."' WHERE `l_id` IN($l_id)";
		$query = $this->db->query($sql);
		if($query)
		{
			return true;
		...
		*/
		$sql = "SELECT
					...
					`a`.`a_office` AS sig_officeid,
                                        SUBSTRING_INDEX(`l`.`l_agency`, '-', 1) AS sig_office,
	...
	public function m_get_leaveposition()
	{
		...
		$sql = 'SELECT DISTINCT(SUBSTRING_INDEX(`l_agency`,"-",1))  AS `l_agency` FROM `tbl_leaves` WHERE `l_agency` != "" ORDER BY `l_agency` ASC';
		$query = $this->db->query($sql);
		if($query->num_rows() > 0)
		{
			return $query->result();
