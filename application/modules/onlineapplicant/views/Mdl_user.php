<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
		...
		public function m_login($username,$password)
		{
			...
			$newpass = sha1(md5($password.'c[x]t!@n>[*]{<Lo[R];eN}Ce}'));
				...
				// $sql = "SELECT `a`.`a_id`,
								...
								// ) IS NULL,'HR_ERROR',(SELECT `wi`.`p_appointment`
									// FROM `tbl_workinfo` `wi` 
										// WHERE `wi`.`a_id` = `a`.`a_id`
											// AND (`wi`.`p_to` IS NULL OR `wi`.`p_to` = '0000-00-00')
									// AND `wi`.`p_isservicerecord` = 'YES'
	...
	`w`.`p_salarystep` AS `a_salarystep`,
		...
		$sql = "SELECT * FROM `tbl_educbg` WHERE `a_id` = $a_id ORDER BY `pi_from` ASC";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0)
		{
			return $query->result();			
		...
		$sql = "SELECT * FROM `tbl_training` WHERE `a_id` = $a_id ORDER BY `t_from` ASC";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0)
		{
			return $query->result();			
		...
		$sql = "SELECT * FROM `tbl_eligibility` WHERE `a_id` = $a_id ORDER BY `el_examdate` ASC";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0)
		{
			return $query->result();			
	...
	public function m_get_holidays(){
		...
		$sql = "SELECT * FROM `tbl_holidays` ORDER BY `h_date` ASC";
		$query = $this->db->query($sql);
		if($query->num_rows() > 0){
			return $query->result();
		}else{
